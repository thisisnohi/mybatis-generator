...
public class DatabaseIntrospector {
     ...
     *            the introspected table
     */
    private void calculatePrimaryKey(FullyQualifiedTable table,IntrospectedTable introspectedTable,String catalog,String schema) {
        ResultSet rs = null;

        try {
            rs = databaseMetaData.getPrimaryKeys(catalog, schema, table.getIntrospectedTableName());
        } catch (SQLException e) {
            closeResultSet(rs);
            warnings.add(getString("Warning.15")); //$NON-NLS-1$
            return;
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(
        ...
        ResultSet rs = null;
        System.out.println("localSchema:" + localSchema + ",localCatalog:" + localCatalog);
        if (null == localSchema && null != localCatalog && !"".equals(localCatalog)) {
        	rs = databaseMetaData.getColumns(localCatalog, localCatalog,localTableName, "%"); //$NON-NLS-1$
        }else {
    ...
    private List<IntrospectedTable> calculateIntrospectedTables(TableConfiguration tc,Map<ActualTableName, List<IntrospectedColumn>> columns) {
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
            ...
            //20170404 edit by nohi 修改catlog取值
