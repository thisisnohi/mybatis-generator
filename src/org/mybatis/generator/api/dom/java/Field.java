...
public class Field extends JavaElement {
	...
	private boolean isId;//是否是ID

	private String remark;// 注释
	private String actualColumnName;//实际列表
	 ...
	 * @return
	 */
	public void addFormatedRemarks(StringBuilder sb,int indentLevel) {
...
//		addFormatedRemarks(sb,indentLevel);
		addFormattedJavadoc(sb, indentLevel);
		addFormattedAnnotations(sb, indentLevel);

		String jpaFlagStr = PropertiesUtils.getInstance().getConfigValue("Jpa.flag");
		String columnFlagStr = PropertiesUtils.getInstance().getConfigValue("Jpa.field.column.flag");
		boolean jpaFlag = "true".equalsIgnoreCase(jpaFlagStr) ? true : false;
		boolean columnFlag = "true".equalsIgnoreCase(columnFlagStr) ? true : false;
		if (jpaFlag && columnFlag) {
			OutputUtilities.javaIndent(sb, indentLevel);
			if (!isId) {
				sb.append("@Column(name=\"").append(actualColumnName).append("\")");
			}else {
				sb.append("@Id");
			}
