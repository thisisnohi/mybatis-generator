...
public class MyBatisGenerator {
	...
	public void generate(ProgressCallback callback, Set<String> contextIds, Set<String> fullyQualifiedTableNames, boolean writeFiles) throws SQLException, IOException, InterruptedException {
		...
		System.out.println("MyBatisGenerator.contextsToRun:" + contextsToRun);
	...
	private void writeGeneratedJavaFile(GeneratedJavaFile gjf, ProgressCallback callback) throws InterruptedException, IOException {
		...
		String source;
		try {
			boolean isOverWriter = false;
			String overWriter = PropertiesUtils.getInstance().getConfigValue("MyBatisGenerator.isOverwriteEnabled");
			if ("true".equalsIgnoreCase(overWriter)) {
				isOverWriter = true;
			}
			...
			if (targetFile.exists()) {
				...
				} else if (isOverWriter || shellCallback.isOverwriteEnabled()) {
