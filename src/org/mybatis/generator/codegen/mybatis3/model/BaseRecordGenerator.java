...
public class BaseRecordGenerator extends AbstractJavaGenerator {
	...
	public List<CompilationUnit> getCompilationUnits() {
		...
		System.out.println("PrimaryKeyColumns:" + list);
		List<String> primaryKeyColumn = new ArrayList<String>();
		for (IntrospectedColumn item : list) {
			primaryKeyColumn.add(item.getActualColumnName());
		}
		System.out.println("PrimaryKeyColumns:" + primaryKeyColumn);
		for (IntrospectedColumn introspectedColumn : introspectedColumns) {
			if (RootClassInfo.getInstance(rootClass, warnings).containsProperty(introspectedColumn)) {
				continue;
			}
			Field field = getJavaBeansField(introspectedColumn, context, introspectedTable);
			if (primaryKeyColumn.contains(introspectedColumn.getActualColumnName())) {
				System.out.println("KeyColumns:" + field);
				field.setId(true);
			}
